...
import static cn.piesat.sanitation.util.carmera.GlobalSet.FEATURE_SIZE;
import static cn.piesat.sanitation.util.carmera.GlobalSet.TIME_TAG;
...
//            ToastUtil.show(context, "未授权设备，请完成授权激活");
            if (listener != null) {
...
//        ToastUtil.show(context, "模型初始化中，请稍后片刻");
        ...
        faceDetect.initModel(context,
                ...
                new Callback() {
                    @Override
                    public void onResponse(int code, String response) {
                        //  ToastUtil.show(context, code + "  " + response);
...
//                            ToastUtil.show(context, "模型加载完毕，欢迎使用");
                            if (listener != null) {
