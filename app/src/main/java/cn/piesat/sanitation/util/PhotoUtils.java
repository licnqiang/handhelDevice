...
public class PhotoUtils {
     ...
     * 使用方式:
     *
     * 1.获取拍照原图Bitmap:  PhotoUtils.onInitPhoneResult(this,requestCode,resultCode,data,"init.jpg")
     *   参数列表：上下文环境，请求码，响应码，返回的数据，图片名称
     *   注：请求码，响应码，返回的数据是onActivityResult（）方法的参数
     * 2.获取一定压缩比例的原图
     *   （1）方式一：拍照保存：PhotoUtils.onInitPhoneResult(this,requestCode,resultCode,data,"init.jpg")
     *                将拍照保存路径下的照片进行处理  PhotoUtils.getBitmapOption(Environment.getExternalStorageDirectory() + "/phone/init.jpg", 20))
     *                参数：拍照保存的全路径，缩放比列（20表示缩放比为原图的1/20）
     *   （2）方式二：将现有的bitmap进行压缩处理：
     *                将bitmap保存：PhotoUtils.saveBitmapFile(bitmap,filePath);
     *                   参数：有效的bitmap，保存的全路径
     *                将保存路径下的bitmap进行压缩：PhotoUtils.getBitmapOption(filePath, 20))
     *                   参数：保存的全路径，缩放比列
     * 3.保存一个bitmap:PhotoUtils.saveBitmapFile(bitmap,filePath);
     *                   参数：有效的bitmap，保存的全路径
     * 4.缩放一个一个路径下的bitmap：PhotoUtils.getBitmapOption(filePath, 20))
     *                   参数：保存的全路径，缩放比列
     * 5.获取一个拍照缩略图：
     *                拍照：PhotoUtils.getAbbreviationsPhone(this);
     *                在onActivityResult（）中PhoneUtils.onAbbreviationsPhoneResult(this,requestCode,resultCode,data)
     *                   参数：上下文，请求码，响应码，返回数据
     * 6.获取相册选择的单个图片：
     *   （1）：得到一个图片的bitmap：
     *          调用相册：PhotoUtils.getAlbumPhone(上下文环境);
     ...
     *      在在onActivityResult（）中监听PHONE_CODE_SHEAR_PACK响应码调用Uri packUri =PhotoUtils.onInitCutPhoneResult(this,requestCode,resultCode,data,"cut.jpg");获得拍照的原图
     ...
     *      在在onActivityResult（）中监听PHONE_CODE__SHEAR_MYPHONE响应码调用Uri myUri=PhotoUtils.onCutmyPhoneResult(this,requestCode,resultCode,data);;获得相册图片uri
